...
BOOL CAdbshell::Start()
{
	...
	//CString* str = new CString("adb.exe shell");
	WCHAR prog[] = L"D:\\adb\\4.4\\adb.exe";
	WCHAR args[] = L"adb.exe shell";
	if (CreateProcess((LPWSTR)prog, (LPWSTR)args, NULL, NULL, TRUE, 0, NULL, NULL, &startupinfo, &pinfo) == FALSE) {
...
CString CAdbshell::GetOutput()
{
	...
	nPos1 = m_strOutput.Find((LPCWSTR)"# exit");
	if (nPos1 == -1) {
		nPos1 = m_strOutput.Find((LPCWSTR)"$ exit");
	}
	if (nPos1 != -1) {
